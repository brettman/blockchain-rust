...
impl Debug for Block {
    fn fmt (&self, f: &mut Formatter) -> fmt::Result {
        write!(f, "Block[{}]: {} at: {} with: {} __ nonce: {} ", 
            &self.index, 
...
impl Hashable for Block {
    ...
    fn bytes(&self) -> Vec<u8>{
        ...
        bytes.extend(
            self.transactions
                .iter()
                .flat_map(|transaction| transaction.bytes())    //i think this should return flattened Vec<u8>
                .collect::<Vec<u8>>()   // do we need this?  seems like previous call returns same type...
        );
